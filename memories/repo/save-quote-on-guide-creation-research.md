...
**Updated:** 2026-06-30 — all 5 open questions resolved; `id` is required
...
- **`id` is required** — the quote identifier needed by the provider API; not optional
...
- **Breaking API change:** `CreateGuideDto` drops top-level `quoteId: string`; client sends `quote.id` instead (required). `callProviderApi` references `payload.quoteId` → must change to `payload.quote?.id`.
- `source` intentionally excluded from snapshot — avoids duplicating `Guide.provider`.
- `QuoteSnapshot.id` is **required** — the quote identifier needed by the provider API. Not optional.
