...
- Charging `retryFailedGuide`; retry must never invoke wallet debit (AC 6).
...
| AC 6 - precheck, provisional guide, guarded debit, provider call, retained debit, no retry charge | Phase 5 |
...
Use existing guide states instead of adding a new enum value:
...
- If guarded debit returns `null`, abort the transaction, update the guide to `status: 'failed'` plus `balanceChargeStatus: 'insufficient'` and the balance insufficient-funds `failureInfo.errorCode`, then throw the same business error without calling the provider.
- Provider success/failure finalizes the provisional guide as `created`/`failed` using the existing provider result mapping.
- Extend retry eligibility to require `status: 'failed'`; new guides are retryable only when `balanceChargeStatus === 'debited'`. Explicitly reject `pending`, `insufficient`, and `bypassed`; legacy failed guides with no charge marker retain their existing retry behavior.

The marker is internal and must not be added to public guide response DTOs.
...
- Fields: required/indexed `userEmail`; required integer/minimum-one/`immutable: true` `amountInCents`; optional `paymentReference`; enum/indexed/default-pending `status`; `adminInCharge: string | null` with explicit default `null`; optional `decisionReason`; optional `decisionAt`.
...
- Inspect both generated schemas for required/default/enum/index contracts, safe integer-cent limits, immutable request amount, the unique wallet owner, and explicit `adminInCharge: null` default.
...
- Admin request item extends the user-visible shape with `userEmail`, `userName`, and nullable `adminInCharge`.
...
- Add `async findAdmins(): Promise<UserDoc[]>` using `userModel.find({ role: 'admin' }).select('email name lastName').exec()`; role is an array and MongoDB array membership handles the scalar query.
- Add `async findByEmails(emails: string[]): Promise<UserDoc[]>` using one `$in` query and selecting `email name lastName`; return `[]` immediately for an empty input.
...
- Cover admin array-membership query, batch `$in` lookup, empty input, selected fields, and asynchronous database failures.
...
**`src/balance/services/balance.service.ts` - Create**
...
1. Add one caller-email helper that rejects missing JWT email; admin methods also validate `role.includes('admin')` in service logic rather than relying only on controller guards.
2. `createRequest`: truncate/convert the request amount to cents, persist `pending` with `adminInCharge: null`, build the success envelope, then attempt requester/admin lookup and email in an isolated `try/catch`. Log mail/lookup failures and still return success.
...
5. `getAllRequests`: validate admin, build the month query, add `status: 'pending'` only when requested, paginate, batch distinct emails through `findByEmails`, and map names as `name + lastName`; fall back to raw email when a user record is missing.
6. `decideRequest`: approval uses the transaction described above; rejection uses `findOneAndUpdate({ _id, status: 'pending' })`. Both record acting admin email/reason/time; only approval overwrites the request's payment reference with the required verified value. The wallet credit query must guard `current amountInCents <= MAX_SAFE_CENTS - credit`; a limit/null/duplicate-key branch rolls back the request decision and maps to a balance business error. Resolve `userName` for the admin response with raw-email fallback, then notify the requester after commit in an isolated `try/catch`; failed lookup/mail cannot make required response fields undefined.
...
- Create truncation, pending/null-admin persistence, many pending requests, envelope, all-admin notification, and best-effort mail failure.
...
- Admin pending/all filters, batch lookup once per page, full-name mapping, and email fallback.
...
**`src/balance/controllers/balance.controller.ts` - Create**
...
**`src/app.module.ts` - Modify imports near `GuidesModule` (current import line 22 and imports-array line 76)**

- Import and register `BalanceModule` once. No environment validation changes are needed.
...
- For new marked guides, return ineligible unless `balanceChargeStatus === 'debited'`; this blocks provisional, insufficient, and mock-bypassed guides. Preserve existing retry behavior for legacy failed guides where the marker is `undefined`.
- Keep the balance insufficient-funds error-code check as defense in depth before count/cooldown checks.
- Do not add any BalanceService call to `retryFailedGuide`; charged provider failures continue directly to provider retry.
...
- Add call-order and branch coverage for initial check, guarded debit, provider invocation, provider failure retention, race failure, mock bypass, and retry behavior.
...
- Provider failure: debit remains, guide is failed and retryable, and retry does not debit again.
- Insufficient-balance failed guide: retry is rejected before provider call.
- `waiting`, insufficient, mock-success, and mock-failure guides are not retryable; mock flows neither read nor debit balance.
...
- Retry of charged provider failure has zero balance method calls; retry of balance-insufficient failure is blocked.
...
- Reuse guide `waiting` for provisional persistence and `failed` plus an internal `balanceChargeStatus` marker for outcomes; no new public guide status is added.
- Commit guarded debit and the guide's charged marker in one transaction, then block retries for pending/insufficient/bypassed markers while leaving charged provider failures and legacy marker-less failures retryable without another debit.
