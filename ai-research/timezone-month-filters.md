...
- Reject date-only guide `startDate` and `endDate` inputs. Date-range clients must submit ISO 8601 date-time instants with an explicit `Z` or numeric offset; this prevents JavaScript from silently interpreting a date-only value as UTC instead of the business calendar.
